...
namespace HospitalWMS.Client.Controls.Purchaser
{
    public partial class PurchaseQueryControl : BaseDataControl
    {
        ...
        private void uiButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                PurchaserMainControl.Instance.SkipUI(typeof(PurchaseManageControl), new Model.EntityBase() { id = (long)dgvApply.Rows[dgvApply.SelectedRows[0].Index].Cells["编号"].Value });
            }
            else
            {
                MessageBox.Show("该记录已审核！");
